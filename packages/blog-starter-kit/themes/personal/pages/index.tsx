...
const showLocalPage = (slug: string | undefined) => slug && ['naming-convention'].includes(slug);

const getInitialPosts = (posts: PostFragment[]) => {
	const result = posts.map((p) => {
		const url = p.url.split('/');
		const slug = url.pop();
		if (showLocalPage(slug)) {
			return { ...p, url: `${url.join('/')}/local/${slug}`, slug: `local/${slug}` };
		}
		return p;
	});
...
export default function Index({ publication, initialPosts, initialPageInfo }: Props) {
	const [posts, setPosts] = useState<PostFragment[]>(getInitialPosts(initialPosts));
